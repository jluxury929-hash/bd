...
const TREASURY_WALLET = '0x86bB004AF573623752401F14D9847917745556bc';
...
let recyclePercentage = 50; // Starts at 50%, decreases to 10% as treasury grows
...
// Starts at 50% of profits, decreases to 10% as treasury grows
// Goal: After initial $175-500 CAD deposit, never need to pay gas again
// ===============================================================================
function calculateRecyclePercentage(treasuryBalance) {
  // Treasury thresholds in ETH
  if (treasuryBalance < 0.02) return 50;      // Low: recycle 50%
  if (treasuryBalance < 0.05) return 40;      // Building: recycle 40%
  if (treasuryBalance < 0.1) return 30;       // Moderate: recycle 30%
  if (treasuryBalance < 0.2) return 20;       // Good: recycle 20%
  if (treasuryBalance < 0.5) return 15;       // Strong: recycle 15%
  return 10;                                   // Excellent: recycle 10%
}

async function autoRecycleFromProfit(profitUSD) {
  if (!autoRecycleEnabled || profitUSD <= 0) return { recycled: 0 };
  
  const balance = await getTreasuryBalance();
  recyclePercentage = calculateRecyclePercentage(balance);
  
  // Calculate recycle amount based on current percentage
  const recycleUSD = profitUSD * (recyclePercentage / 100);
  const recycleETH = recycleUSD / ETH_PRICE;
  
  totalRecycled += recycleUSD;
  
  console.log('[RECYCLE] ' + recyclePercentage + '% of $' + profitUSD.toFixed(2) + ' = $' + recycleUSD.toFixed(2) + ' -> backend gas');
  
  return {
    recycled: recycleUSD,
    recycledETH: recycleETH,
    percentage: recyclePercentage,
...
async function autoRecycleToBackend() {
  ...
  // If treasury is very low, recycle more aggressively
  if (balance >= 0.1) {
    return { success: false, reason: 'Treasury has sufficient gas (' + balance.toFixed(4) + ' ETH)' };
  }
  
  if (totalEarnings < 20) {
    return { success: false, reason: 'Insufficient earnings to recycle (need $20+)' };
  }
  
  // Recycle based on current percentage (min 0.01 ETH, max 0.05 ETH)
  const targetRecycleUSD = totalEarnings * (recyclePercentage / 100);
  const recycleETH = Math.min(0.05, Math.max(0.01, targetRecycleUSD / ETH_PRICE));
  const recycleUSD = recycleETH * ETH_PRICE;
  
  totalEarnings -= recycleUSD;
  totalRecycled += recycleUSD;
  
  console.log('[RECYCLE] Auto-recycled ' + recyclePercentage + '% = $' + recycleUSD.toFixed(0) + ' -> ' + recycleETH.toFixed(4) + ' ETH to backend');
  ...
  return { 
    ...
    treasuryBalance: balance + recycleETH
  };
}
...
app.get('/status', async (req, res) => {
  ...
  res.json({
    ...
    totalRecycledUSD: totalRecycled.toFixed(2),
...
app.post('/execute', async (req, res) => {
  ...
  try {
    ...
    if (flashRes.ok) {
      ...
      // Auto-recycle percentage of profit to backend
      const recycleResult = await autoRecycleFromProfit(profit);
      const netProfit = profit - recycleResult.recycled;
      totalEarnings += netProfit;
      totalStrategiesExecuted++;
      
      console.log('[STRATEGY #' + strategy.id + '] ' + strategy.name + ' | Profit: $' + profit.toFixed(2) + ' | Recycled: $' + recycleResult.recycled.toFixed(2) + ' (' + recycleResult.percentage + '%)');
  ...
  console.log('[STRATEGY #' + strategy.id + '] ' + strategy.name + ' (sim) | Profit: $' + profit.toFixed(2) + ' | Recycled: $' + recycleResult.recycled.toFixed(2) + ' (' + recycleResult.percentage + '%)');
  ...
  res.json({
    ...
    recycledUSD: recycleResult.recycled.toFixed(2),
    recyclePercentage: recycleResult.percentage,
    profitETH: (netProfit / ETH_PRICE).toFixed(6),
    totalEarnings: totalEarnings.toFixed(2),
    totalStrategiesExecuted: totalStrategiesExecuted,
    feeRecipient: COINBASE_WALLET,
    mode: 'simulation'
